...
def load_data(config):
    ...
    target_dim = config['training']['target_dim']
...
def build_model(config, device):
    feature_cols = config['data']['feature_cols']
    target_cols = config['data']['target_cols']
    model = myTransformer(input_dim=len(feature_cols),output_dim=len(target_cols)).to(device)
    return model
...
def evaluate(model, test_loader, device, output_window, feature_cols, target_cols):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x, y = x.to(device), y.to(device)
            tgt_in = torch.zeros((x.size(0), output_window, len(feature_cols)), device=x.device)
            pred = model(x, tgt_in)
...
def main():
    ...
    target_cols = config['data']['target_cols']
    output_window = config['data']['output_window']
    log.info("TRASFORMER HYPER-PARAMETERS:")
    ...
    for epoch in range(epochs):
        train_loss = train(model, train_loader, criterion, optimizer, device,output_window, len(feature_cols))
        val_loss = validate(model, val_loader, criterion, device, output_window, len(feature_cols))
        log.info(f"Epoch {epoch+1}/{epochs}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
    results = evaluate(model, test_loader, device, output_window, feature_cols, target_cols)
    print("\nValutazione finale:")
